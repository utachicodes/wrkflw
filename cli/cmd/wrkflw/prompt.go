...
func buildPrompt(d promptDetails) string {
	...
	write("You are the Wrkflw agent %q (ID %s).", d.AgentName, d.AgentID)
	...
	write("")
	write("    \"$WRKFLW_BIN\" tasks claim %s", d.TaskID)
	write("")
	...
	write("")
	write("    \"$WRKFLW_BIN\" tasks get %s", d.TaskID)
	write("    \"$WRKFLW_BIN\" tasks entries %s", d.TaskID)
	write("")
	...
	write("")
	write("    \"$WRKFLW_BIN\" tasks comment %s \\", d.TaskID)
	write("      --file \"${TMPDIR:-/tmp}/wrkflw-note-$WRKFLW_RUN_ID.md\" \\")
	...
	write("")
	write("    \"$WRKFLW_BIN\" tasks output %s \\", d.TaskID)
	write("      --file \"${TMPDIR:-/tmp}/wrkflw-report-$WRKFLW_RUN_ID.md\" \\")
