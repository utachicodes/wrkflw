...
The migration job and service attach the production Cloud SQL instance in Europe West 1 because `slate-database-url` uses that socket. Deploys replace the complete secret mapping explicitly, including `INVITE_CODE`, so early-access registration is not disabled accidentally.
