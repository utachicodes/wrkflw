...
The boards collapse took `045` through `047`, so agent config starts at `048`.
These numbers are the order the work happens in, not a reservation: whatever
ships first takes the next number, and the rest shift.
...
| C | `047_drop_boards`: drop the `boards` table and both `board_id` columns | The word board survives only as the name of the status view |
...
| # | Task | Done when |
| --- | --- | --- |
| 2.1 | `049_runs` migration, `runs` package, lifecycle transitions | A run row is created, transitions, and terminates |
| 2.2 | `051_run_events`, batched append-only ingest with monotonic seq | Events upload, resume after a network drop, and replay in order |
| 2.3 | Run detail view with event replay | Opening a run shows its event stream |
| 2.4 | `052_task_execution_policy`: `requires_approval`, `attempts`, `target_runner`, `workspace`, `blocked` status | The approval toggle gates the move to Done |
...
| # | Task | Done when |
| --- | --- | --- |
| 3.1 | `050_runners`, registration tokens, register and deregister endpoints | A runner registers and appears in the UI |
...
| # | Task | Done when |
| --- | --- | --- |
| 8.1 | `056_task_schedules`: cron, timezone, next_run_at, spawned_from | A task carries a schedule defaulting to Run once |
