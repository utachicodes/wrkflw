...
const youtubeSteps: TemplateStep[] = [
  { phaseId: "define", title: "Capture the idea", executor: "Human", instruction: "Write down the audience, problem, promise and reason this video should exist." },
  { phaseId: "define", title: "Generate title options", executor: "Agent-ready", instruction: "Generate strong title options from the idea, audience and core promise." },
  { phaseId: "define", title: "Approve the title", executor: "Human", instruction: "Choose the title that makes the clearest promise without overstating the video." },
  { phaseId: "define", title: "Write the outline", executor: "Human", instruction: "Map the argument, examples and teaching sequence before writing the full script." },
  { phaseId: "define", title: "Handwrite the introduction", executor: "Human", instruction: "Write the opening hook in your own words and make the payoff clear." },
  { phaseId: "define", title: "Write the script", executor: "Agent-ready", instruction: "Turn the approved outline and introduction into a complete recording script." },
  { phaseId: "produce", title: "Record the video", executor: "Human", instruction: "Record the approved script and store the raw footage in the content folder." },
  { phaseId: "produce", title: "Edit the video", executor: "Human", instruction: "Edit the recording into the finished YouTube cut." },
  { phaseId: "produce", title: "Export the final MP4", executor: "Human", instruction: "Export the approved master and add the canonical file location as an output." },
  { phaseId: "publish", title: "Generate the transcript", executor: "Automation", instruction: "Transcribe the final MP4 and save the transcript in the content workspace." },
  { phaseId: "publish", title: "Write the YouTube description", executor: "Agent-ready", instruction: "Create the description from the approved title, transcript, CTA and resource links." },
  { phaseId: "publish", title: "Complete the publishing check", executor: "Human", instruction: "Verify the video, title, description and required publishing assets before release." },
  { phaseId: "repurpose", title: "Write the Kit promotional email", executor: "Agent-ready", instruction: "Turn the transcript into a concise email that earns the click to the video." },
  { phaseId: "repurpose", title: "Write the Substack newsletter", executor: "Agent-ready", instruction: "Adapt the transcript into a useful standalone newsletter issue." },
  { phaseId: "repurpose", title: "Write the Substack promotional post", executor: "Agent-ready", instruction: "Create a short native post that promotes the newsletter or video." },
  { phaseId: "repurpose", title: "Write the LinkedIn newsletter", executor: "Agent-ready", instruction: "Adapt the transcript into a clear LinkedIn newsletter in your voice." },
  { phaseId: "repurpose", title: "Create the LinkedIn post or carousel", executor: "Agent-ready", instruction: "Choose the strongest LinkedIn format and turn one useful idea into a native asset." },
]
...
function migrateTemplate(value: unknown): ProcessTemplate | null {
  ...
  const migratedSteps = template.steps.map(value => {
    if (!value || typeof value !== "object") return null
    const step = value as Record<string, unknown>
    const phaseId = typeof step.phaseId === "string" ? step.phaseId : migratedPhases.find(phase => phase.name === step.phase)?.id
    if (!phaseId || typeof step.title !== "string" || typeof step.instruction !== "string" || typeof step.executor !== "string" || !["Human", "Agent-ready", "Automation"].includes(step.executor)) return null
    return { phaseId, title: step.title, instruction: step.instruction, executor: step.executor as Executor }
  })
  if (!migratedPhases.length || migratedSteps.some(step => !step)) return null
...
function loadTemplates(key: string): ProcessTemplate[] {
  try {
    ...
    if (!Array.isArray(value) || !value.length) return [cloneTemplate(youtubeTemplate)]
    const migrated = value.map(migrateTemplate)
    return migrated.every(Boolean) ? migrated as ProcessTemplate[] : [cloneTemplate(youtubeTemplate)]
  } catch {
...
function blankTemplate(): ProcessTemplate {
  ...
  return { id: crypto.randomUUID(), name: "", summary: "", taskPrefix: "Run", phases: [phase], steps: [{ phaseId: phase.id, title: "", executor: "Human", instruction: "" }] }
}
  ...
  const createFromTemplate = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        const context = [
          `Created from template: ${selectedTemplate.name}`,
          brief.trim(),
          "",
        ...
        parent = await api.post<Task>(endpoint, {
          title: `${selectedTemplate.taskPrefix || selectedTemplate.name}: ${taskTitle.trim()}`,
          description: context,
          kind: "action",
          status: "new",
          priority: "p1",
        ...
        for (const [index, step] of selectedSteps.entries()) {
          const phaseName = selectedTemplate.phases.find(phase => phase.id === step.phaseId)?.name || "Process"
    ...
    <div className="page-wrap template-page">
      ...
      <section className="template-library" aria-label="Available templates">
        {templates.map(template => <article className={`template-list-row surface-card ${template.id === selectedTemplate.id ? "is-selected" : ""}`} key={template.id}>
          <button type="button" className="template-list-select" onClick={() => setSelectedTemplateId(template.id)} aria-pressed={template.id === selectedTemplate.id}>
          ...
          <div className="template-list-actions"><Button variant="ghost" size="sm" onClick={() => openEditor(template)}><Pencil className="size-3.5" />Edit</Button><Button variant="secondary" size="sm" className="template-use-button" onClick={() => openTemplate(template)}><Play className="size-3.5" />Use template</Button></div>
        </article>)}
      </section>
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}>
        <DialogContent className="template-editor-dialog" showClose={false} aria-describedby={undefined}>
          {editorDraft && <form onSubmit={event => { event.preventDefault(); saveEditor() }}>
            <DialogHeader className="template-dialog-header"><div className="template-dialog-title"><div className="template-icon small"><Workflow aria-hidden="true" /></div><div><DialogTitle>{templates.some(template => template.id === editorDraft.id) ? "Edit template" : "New template"}</DialogTitle><DialogDescription>Define the phases and ordered subtasks in this process.</DialogDescription></div><button type="button" onClick={() => setEditorOpen(false)} aria-label="Close"><X aria-hidden="true" /></button></div></DialogHeader>
            <div className="template-editor-body">
              ...
              <div className="template-editor-section-head"><div><strong>Process</strong><span>{orderedTemplateSteps(editorDraft).length} ordered subtasks</span></div><Button type="button" variant="ghost" size="sm" onClick={() => { const phase = { id: crypto.randomUUID(), name: `Phase ${editorDraft.phases.length + 1}` }; setEditorDraft({ ...editorDraft, phases: [...editorDraft.phases, phase], steps: [...editorDraft.steps, { phaseId: phase.id, title: "", executor: "Human", instruction: "" }] }) }}><Plus className="size-3.5" />Add phase</Button></div>
                ...
                {editorDraft.phases.map((phase, phaseIndex) => {
                  ...
                  return <section className="template-editor-phase" key={phase.id}>
                    ...
                    <div className="template-editor-steps">
                      {phaseSteps.map((step, stepIndex) => <div className="template-editor-step" key={stepIndex}>
                      ...
                      <Button type="button" variant="ghost" size="sm" className="template-add-step" onClick={() => setEditorDraft({ ...editorDraft, steps: [...editorDraft.steps, { phaseId: phase.id, title: "", executor: "Human", instruction: "" }] })}><Plus className="size-3.5" />Add subtask</Button>
                    </div>
                  </section>
                })}
              </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={closeDialog}>
        <DialogContent className="template-create-dialog" showClose={false} aria-describedby={undefined}>
          <form onSubmit={event => { event.preventDefault(); if (taskTitle.trim() && !partialTask) createFromTemplate.mutate() }}>
            ...
            <div className="template-form-grid">
              <div className="template-form-wide"><Label htmlFor="template-task-title">Task name</Label><Input id="template-task-title" value={taskTitle} onChange={event => setTaskTitle(event.target.value)} placeholder="This week’s run" autoFocus required disabled={createFromTemplate.isPending || Boolean(partialTask)} /></div>
              <div><Label htmlFor="template-planned-date">Plan for</Label><span className="date-property"><CalendarDays aria-hidden="true" /><Input id="template-planned-date" type="date" value={plannedDate} onChange={event => setPlannedDate(event.target.value)} disabled={createFromTemplate.isPending || Boolean(partialTask)} /></span></div>
              ...
              <div className="template-form-wide"><Label htmlFor="template-brief">Brief</Label><Textarea id="template-brief" value={brief} onChange={event => setBrief(event.target.value)} placeholder="Add the context for this run" disabled={createFromTemplate.isPending || Boolean(partialTask)} /></div>
            </div>
            ...
            <DialogFooter className="template-dialog-footer">
              {partialTask ? <Button type="button" onClick={() => { setDialogOpen(false); navigate(taskUrl(partialTask)) }}>Open partial task</Button> : <><Button type="button" variant="ghost" onClick={() => closeDialog(false)} disabled={createFromTemplate.isPending}>Cancel</Button><Button type="submit" disabled={!taskTitle.trim() || createFromTemplate.isPending}>{createFromTemplate.isPending ? `Creating ${creatingStep || 1}/${selectedSteps.length}` : "Create task"}</Button></>}
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
