...
type TaskDrop = { id: string; status: TaskStatus; referenceTaskId?: string; placement?: "before" | "after"; position?: number; preservePosition?: boolean; append?: boolean }

export function taskDropLocation(tasks: Task[], draggedID: string, targetID?: string, after = false): Pick<TaskDrop, "referenceTaskId" | "placement" | "position" | "append"> | undefined {
...
export function orderedTaskSiblings(tasks: Task[], moved: Task) {
  return tasks.filter(task => task.bucketId === moved.bucketId && task.id !== moved.id).sort((left, right) => (left.sortOrder ?? 0) - (right.sortOrder ?? 0))
}

export function optimisticTaskMove(tasks: Task[], { id, status, referenceTaskId, placement, position = 0, preservePosition, append }: TaskDrop) {
  const moved = tasks.find(task => task.id === id)
  if (!moved) return tasks
  const siblings = orderedTaskSiblings(tasks, moved)
  ...
  const nextPosition = preservePosition ? currentPosition : append ? siblings.length : referenceIndex < 0 ? position : referenceIndex + (placement === "after" ? 1 : 0)
  const ordered = [...siblings.slice(0, nextPosition), { ...moved, status }, ...siblings.slice(nextPosition)]
  const updates = new Map(ordered.map((task, index) => [task.id, { ...task, sortOrder: index }]))
...
export function WorkspacePage() {
  ...
  const dropTask = useMutation({
    mutationFn: async ({ id, status, ...location }: TaskDrop) => {
      const task = tasks.find(item => item.id === id)
      if (!task) throw new Error("Task not found")
      return api.post<Task>(`/api/v1/tasks/${encodeURIComponent(id)}/move`, { bucketId: task.bucketId, status, ...location })
    },
    onMutate: input => {
      queryClient.setQueriesData<InfiniteData<TasksPage, string>>({ queryKey: ["tasks"] }, old => {
        if (!old) return old
        const updated = new Map(optimisticTaskMove(old.pages.flatMap(page => page.tasks), input).map(task => [task.id, task]))
        return { ...old, pages: old.pages.map(page => ({ ...page, tasks: page.tasks.map(task => updated.get(task.id) || task) })) }
      })
    },
    onSuccess: refresh,
    onError: error => { setPageError(error instanceof Error ? error.message : "Could not move task"); void refresh() },
  })
