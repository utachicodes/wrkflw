...
import { taskListName, type Agent, type Entry, type Task, type User } from "@/lib/types"
...
const emptyGatewayConfig: GatewayConfig = {
  channel: "", agent: "",
...
function MessagingSettings() {
  ...
  const [draft, setDraft] = React.useState<GatewayConfig | null>(null)
  const query = useQuery({ queryKey: ["gateway-config"], queryFn: () => api.get<GatewayConfig>("/api/v1/gateway/config") })
  const saved: GatewayConfig = query.data || emptyGatewayConfig
  const form: GatewayConfig = draft || saved
  const set = (patch: Partial<GatewayConfig>) => setDraft({ ...form, ...patch })
  const save = useMutation({
    mutationFn: () => api.patch<GatewayConfig>("/api/v1/gateway/config", { ...form, routes: [] }),
    onSuccess: async data => { setDraft(null); setNotice("Gateway config saved. The daemon picks it up on its next pull."); await queryClient.invalidateQueries({ queryKey: ["gateway-config"] }); void data },
    onError: error => setNotice(error instanceof Error ? error.message : "Could not save gateway config"),
  })
  const create = useMutation({ mutationFn: () => api.post<{ token: string }>("/api/v1/api-tokens", { name: "gateway mirror" }), onSuccess: async data => { setToken(data.token); setNotice(""); await queryClient.invalidateQueries({ queryKey: ["api-tokens"] }) }, onError: error => setNotice(error instanceof Error ? error.message : "Could not create token") })
  const handle = botName.trim().replace(/^@/, "")
  const validBot = /^[A-Za-z0-9_]{5,32}$/.test(handle)
  const link = validBot ? `https://t.me/${handle}` : ""
  const baseUrl = typeof window !== "undefined" ? window.location.origin : "https://wrkflw"
  const localFile = `assistant_root = "~/Code/assistant"\n\n[wrkflw]\nbase_url = "${baseUrl}"\n${token ? `token = "${token}"` : `# token = "create one below"`}\npull_config = true\nmirror = true\n`
  const copyConfig = async () => { await navigator.clipboard.writeText(localFile); setCopied(true); setTimeout(() => setCopied(false), 2000) }
  const csv = (value: string) => value.split(",").map(entry => entry.trim()).filter(entry => entry !== "")
  const joinIds = (ids: number[]) => ids.join(", ")
  return <div className="max-w-[850px] space-y-5"><section><div className="mb-3"><h2 className="font-semibold">Channel</h2><p className="mt-1 text-sm text-muted-foreground">The gateway pulls this on start and picks up edits on its next pull. {saved.lastPulledAt ? `Last pulled ${new Date(saved.lastPulledAt).toLocaleString()}.` : "Not pulled by any gateway yet."} New here? <a className="font-semibold text-primary" href="https://github.com/utachicodes/wrkflw/blob/main/docs/messaging-setup.md" target="_blank" rel="noreferrer">Five-minute setup guide.</a></p></div><div className="surface-card space-y-4 p-4">{query.isPending ? <p className="text-sm text-muted-foreground">Loading…</p> : <><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="gw-channel">Channel</Label><Select id="gw-channel" value={form.channel} onChange={event => set({ channel: event.target.value })}><option value="">Not configured</option><option value="telegram">Telegram</option><option value="slack">Slack</option><option value="imessage">iMessage</option></Select></div><div><Label htmlFor="gw-agent">Agent backend</Label><Select id="gw-agent" value={form.agent} onChange={event => set({ agent: event.target.value })}><option value="">Default</option><option value="codex">Codex</option><option value="claude">Claude Code</option><option value="pi">Pi</option></Select></div></div><div><Label htmlFor="gw-tg-token">Telegram bot token</Label><Input id="gw-tg-token" type="password" value={form.telegram.botToken} onChange={event => set({ telegram: { ...form.telegram, botToken: event.target.value } })} placeholder="Token from @BotFather" autoComplete="off" /></div><div><Label htmlFor="gw-tg-ids">Telegram allowed user IDs</Label><Input id="gw-tg-ids" value={joinIds(form.telegram.allowUserIds)} onChange={event => set({ telegram: { ...form.telegram, allowUserIds: csv(event.target.value).filter(entry => /^\d+$/.test(entry)).map(Number) } })} placeholder="123456789, 987654321" inputMode="numeric" autoComplete="off" /></div><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="gw-slack-app">Slack app token</Label><Input id="gw-slack-app" type="password" value={form.slack.appToken} onChange={event => set({ slack: { ...form.slack, appToken: event.target.value } })} placeholder="xapp-…" autoComplete="off" /></div><div><Label htmlFor="gw-slack-bot">Slack bot token</Label><Input id="gw-slack-bot" type="password" value={form.slack.botToken} onChange={event => set({ slack: { ...form.slack, botToken: event.target.value } })} placeholder="xoxb-…" autoComplete="off" /></div></div><div><Label htmlFor="gw-slack-ids">Slack allowed user IDs</Label><Input id="gw-slack-ids" value={form.slack.allowUserIds.join(", ")} onChange={event => set({ slack: { ...form.slack, allowUserIds: csv(event.target.value) } })} placeholder="U012ABCDEF" autoComplete="off" /></div><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="gw-im-self">iMessage self handles</Label><Input id="gw-im-self" value={form.imessage.selfHandles.join(", ")} onChange={event => set({ imessage: { ...form.imessage, selfHandles: csv(event.target.value) } })} placeholder="you@icloud.com" autoComplete="off" /></div><div><Label htmlFor="gw-im-from">iMessage allowed senders</Label><Input id="gw-im-from" value={form.imessage.allowFrom.join(", ")} onChange={event => set({ imessage: { ...form.imessage, allowFrom: csv(event.target.value) } })} placeholder="+1555…" autoComplete="off" /></div></div><div className="flex items-center justify-between gap-2"><div>{notice && <p className={`status-message ${save.isError || create.isError ? "error" : ""}`} role={save.isError || create.isError ? "alert" : "status"}>{notice}</p>}</div><Button type="button" onClick={() => save.mutate()} disabled={save.isPending || draft === null}>{save.isPending ? "Saving…" : "Save channel config"}</Button></div></>}</div></section><section><div className="mb-3"><h2 className="font-semibold">Pair your phone</h2><p className="mt-1 text-sm text-muted-foreground">Type the bot username from @BotFather and scan the code to open the chat.</p></div><div className="surface-card p-4"><div><Label htmlFor="telegram-bot-name">Bot username</Label><Input id="telegram-bot-name" value={botName} onChange={event => setBotName(event.target.value)} placeholder="my_assistant_bot" autoComplete="off" /></div>{validBot ? <div className="qr-connect"><div className="qr-frame"><QRCode value={link} size={180} aria-label={`QR code linking to ${link}`} /></div><div><strong>Scan to open the chat</strong><p className="mt-1 text-sm text-muted-foreground">Point your phone camera at the code, open <code>{link}</code>, and send the bot one message.</p></div></div> : <p className="mt-3 text-sm text-muted-foreground">Enter a valid bot username (5–32 letters, numbers or underscores) to generate the QR code.</p>}</div></section><section><div className="mb-3"><h2 className="font-semibold">Connect the daemon</h2><p className="mt-1 text-sm text-muted-foreground">One small file on the machine running the gateway. Everything else lives above.</p></div><div className="surface-card space-y-4 p-4"><div><div className="mb-2 flex items-center justify-between gap-2"><strong className="text-sm">Local file: $FRWRD_HOME/config.toml</strong><Button type="button" variant="secondary" size="sm" onClick={copyConfig}>{copied ? "Copied" : "Copy file"}</Button></div><pre className="overflow-auto rounded-lg bg-muted p-3 text-xs leading-6"><code>{localFile}</code></pre></div><div><div className="mb-2 flex items-center justify-between gap-2"><strong className="text-sm">Mirror token</strong>{!token && <Button type="button" variant="secondary" size="sm" onClick={() => create.mutate()} disabled={create.isPending}>{create.isPending ? "Creating…" : "Create token"}</Button>}</div>{token ? <p className="text-sm text-muted-foreground">Token created and already placed in the file above. It is shown once — copy the file now.</p> : <p className="text-sm text-muted-foreground">Creates a personal API token and drops it straight into the file above.</p>}</div></div></section></div>
}

const agentSetupPrompt = `## wrkflw workflow

You do work that lives on a wrkflw board. The token is already in your
environment as WRKFLW_API_TOKEN. Never print it, never write it to a file.

- Run \`wrkflw tasks pull\` to find queued work.
- Before starting a task, run \`wrkflw tasks claim <task-id>\`.
- Only continue when the claim succeeds. A failed claim means another
  agent took it; pick different work.
- Read full context with \`wrkflw tasks get <task-id>\`.
- Record progress with \`wrkflw tasks comment <task-id> --file <note-file>\`.
- When the work is complete, report with
  \`wrkflw tasks output <task-id> --file <report-file> --idempotency-key <stable-key>\`.
  Output moves the card to Review in the same operation.
- Under a watcher run you cannot set status yourself: claim reaches
  In Progress and output reaches Review. Started by hand, use
  \`wrkflw tasks status <task-id> needs_review\` (and \`done\` after acceptance).
- Treat wrkflw command output as JSON. Preserve IDs exactly.
- Reuse one --idempotency-key when retrying after an uncertain result;
  no duplicate is created.
- Poll no faster than once every five seconds, slow down while idle, and
  respect Retry-After after a 429.
- Write report files outside the repository so they do not sit in the
  working tree as uncommitted changes.`;

function AgentSetupPrompt() {
  const [copied, setCopied] = React.useState(false)
  const copy = async () => {
    await navigator.clipboard.writeText(agentSetupPrompt)
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }
  return <section><div className="mb-3"><h2 className="font-semibold">Agent setup prompt</h2><p className="mt-1 text-sm text-muted-foreground">Paste this into any AI agent chat or into CLAUDE.md / AGENTS.md. It carries no secrets: the agent is expected to already have WRKFLW_API_TOKEN in its environment.</p></div><div className="surface-card space-y-3 p-4"><pre className="overflow-auto rounded-lg bg-muted p-3 text-xs leading-6"><code>{agentSetupPrompt}</code></pre><div><Button type="button" variant="secondary" size="sm" onClick={copy}>{copied ? "Copied" : "Copy prompt"}</Button></div></div></section>
}
