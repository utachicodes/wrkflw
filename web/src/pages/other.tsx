...
export function AgentsPage() {
  const { agents, maxAgents } = useApp()
  return <div className="page-wrap"><PageHeader title="Agents" actions={<Button asChild disabled={agents.filter(agent => !agent.archived).length >= maxAgents}><Link to="/app/agents/new"><Plus className="size-4" />New agent</Link></Button>} />{agents.length ? <section className="surface-card">{agents.map(agent => <Link to={`/app/agents/${encodeURIComponent(agent.id)}`} className="agent-row" data-agent-link={agent.id} key={agent.id}><div className="agent-name"><div className="avatar">{initials(agent.displayName)}</div><span><strong>{agent.displayName}</strong><small>{agent.purpose || "No purpose set"}</small></span></div><div className="flex items-center gap-3 text-xs text-muted-foreground"><span>{Object.values(agent.workCounts || {}).reduce<number>((sum, value) => sum + Number(value || 0), 0)} tasks</span><ChevronLeft className="size-4 rotate-180" /></div></Link>)}</section> : <div className="empty-state"><div><Bot /><h2 className="font-serif text-2xl text-foreground">Bring your first agent.</h2><p>Create an agent identity, then assign work from any task.</p><Button className="mt-4" asChild><Link to="/app/agents/new">Create agent</Link></Button></div></div>}</div>
}
