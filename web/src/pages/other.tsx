...
function MessagingSettings() {
  ...
  const numericId = userId.trim()
  const validId = /^\d+$/.test(numericId)
  ...
  const config = `channel = "telegram"\nagent = "codex"\nassistant_root = "~/Code/assistant"\n\n[telegram]\nbot_token = "paste-the-token-from-BotFather"\nallow_user_ids = [${validId ? numericId : "123456789"}]\n\n[wrkflw]\nbase_url = "${baseUrl}"\n${token ? `token = "${token}"` : `# token = "create one below"`}\nmirror = true\n`
  const copyConfig = async () => { await navigator.clipboard.writeText(config); setCopied(true); setTimeout(() => setCopied(false), 2000) }
  return <div className="max-w-[850px] space-y-5"><section><div className="mb-3"><h2 className="font-semibold">Telegram</h2><p className="mt-1 text-sm text-muted-foreground">Fill in two fields, copy one file. Everything the gateway needs, generated here.</p></div><div className="surface-card space-y-4 p-4"><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="telegram-bot-name">Bot username</Label><Input id="telegram-bot-name" value={botName} onChange={event => setBotName(event.target.value)} placeholder="my_assistant_bot" autoComplete="off" /><p className="mt-1.5 text-xs text-muted-foreground">From @BotFather → /newbot.</p></div><div><Label htmlFor="telegram-user-id">Your numeric Telegram user ID</Label><Input id="telegram-user-id" value={userId} onChange={event => setUserId(event.target.value.replace(/\D/g, ""))} placeholder="123456789" inputMode="numeric" autoComplete="off" /><p className="mt-1.5 text-xs text-muted-foreground">From a getUpdates response after messaging the bot.</p></div></div>{validBot ? <div className="qr-connect"><div className="qr-frame"><QRCode value={link} size={180} aria-label={`QR code linking to ${link}`} /></div><div><strong>Scan to open the chat</strong><p className="mt-1 text-sm text-muted-foreground">Point your phone camera at the code, open <code>{link}</code>, and send the bot one message.</p></div></div> : <p className="text-sm text-muted-foreground">Enter a valid bot username (5–32 letters, numbers or underscores) to generate the QR code.</p>}<div><div className="mb-2 flex items-center justify-between gap-2"><strong className="text-sm">Gateway config</strong><Button type="button" variant="secondary" size="sm" onClick={copyConfig}>{copied ? "Copied" : "Copy config"}</Button></div><pre className="overflow-auto rounded-lg bg-muted p-3 text-xs leading-6"><code>{config}</code></pre><p className="mt-1.5 text-xs text-muted-foreground">Paste into <code>$FRWRD_HOME/config.toml</code> on the machine running the gateway, replacing the bot token. The gateway keeps its secrets in that one file — the app never holds them.</p></div><div><div className="mb-2 flex items-center justify-between gap-2"><strong className="text-sm">Mirror token</strong>{!token && <Button type="button" variant="secondary" size="sm" onClick={() => create.mutate()} disabled={create.isPending}>{create.isPending ? "Creating…" : "Create token"}</Button>}</div>{token ? <p className="text-sm text-muted-foreground">Token created and already placed in the config above. It is shown once — copy the config now.</p> : <p className="text-sm text-muted-foreground">Creates a personal API token and drops it straight into the config above. No trip to the API tab.</p>}{notice || create.isError ? <p className="status-message error mt-2" role="alert">{notice || (create.error instanceof Error ? create.error.message : "Could not create token")}</p> : null}</div></div></section><section><div className="mb-3"><h2 className="font-semibold">iMessage and Slack</h2><p className="mt-1 text-sm text-muted-foreground">These channels cannot use QR pairing. iMessage works only on the Mac running the gateway with Full Disk Access granted. Slack connects through Socket Mode with an installed workspace app and an explicit user allowlist. See the gateway docs for both.</p></div></section></div>
}
