...
IMAGE="$REGION-docker.pkg.dev/$PROJECT_ID/wrkflw/wrkflw:manual-$(git rev-parse --short HEAD)"
CLOUD_SQL_INSTANCES="$PROJECT_ID:$REGION:wrkflw-postgres-ew1"
...
RUNTIME_SECRETS="DATABASE_URL=wrkflw-database-url:latest,SESSION_SECRET=wrkflw-session-secret:latest,RESEND_API_KEY=wrkflw-resend-api-key:latest"
...
existing_service="$(gcloud_as_deploy run services list --region "$REGION" --filter='metadata.name=wrkflw' --format='value(metadata.name)')"
existing_env_names=""
if [ "$existing_service" = wrkflw ]; then
  existing_env_names="$(gcloud_as_deploy run services describe wrkflw --region "$REGION" --format='value(spec.template.spec.containers[0].env[].name)')"
fi
invite_secret_state="$(gcloud_as_deploy secrets versions describe latest --secret=wrkflw-invite-code --format='value(state)' 2>/dev/null || true)"
if [ "$invite_secret_state" = ENABLED ]; then
  RUNTIME_SECRETS="$RUNTIME_SECRETS,INVITE_CODE=wrkflw-invite-code:latest"
elif printf '%s' "$existing_env_names" | tr ';' '\n' | grep -Fx INVITE_CODE >/dev/null; then
  printf '%s\n' 'The live service uses INVITE_CODE, but wrkflw-invite-code:latest is not enabled or accessible' >&2
  exit 1
fi
gcloud_as_deploy run jobs deploy wrkflw-migrate \
  --image "$IMAGE" \
  --region "$REGION" \
  --command /app/wrkflw \
  --args migrate \
  ...
  --set-secrets DATABASE_URL=wrkflw-database-url:latest \
  ...
  --task-timeout 10m \
  --quiet
effective_migration_identity="$(gcloud_as_deploy run jobs describe wrkflw-migrate --region "$REGION" --format='value(spec.template.spec.template.spec.serviceAccountName)')"
...
gcloud_as_deploy run jobs execute wrkflw-migrate --region "$REGION" --wait --quiet
gcloud_as_deploy run deploy wrkflw \
  --image "$IMAGE" \
  --region "$REGION" \
  --platform managed \
...
deployed="$(gcloud_as_deploy run services describe wrkflw --region "$REGION" --format='value(spec.template.spec.containers[0].image)')"
if [ "$deployed" != "$IMAGE" ]; then
  printf 'Expected deployed image %s, got %s\n' "$IMAGE" "$deployed" >&2
  exit 1
fi
effective_max="$(gcloud_as_deploy run services describe wrkflw --region "$REGION" --format="value(metadata.annotations.'run.googleapis.com/maxScale')")"
if [ "$effective_max" != "$MAX_INSTANCES" ]; then
  printf 'Expected maximum instances %s, got %s\n' "$MAX_INSTANCES" "$effective_max" >&2
  exit 1
fi
effective_web_identity="$(gcloud_as_deploy run services describe wrkflw --region "$REGION" --format='value(spec.template.spec.serviceAccountName)')"
...
gcloud_as_deploy run jobs deploy wrkflw-cleanup \
  --image "$IMAGE" \
  --region "$REGION" \
  --command /app/wrkflw \
  --args cleanup \
  ...
  --task-timeout 5m \
  --quiet
effective_cleanup_identity="$(gcloud_as_deploy run jobs describe wrkflw-cleanup --region "$REGION" --format='value(spec.template.spec.template.spec.serviceAccountName)')"
...
cleanup_uri="https://run.googleapis.com/v2/projects/$PROJECT_ID/locations/$REGION/jobs/wrkflw-cleanup:run"
gcloud_as_deploy run jobs add-iam-policy-binding wrkflw-cleanup --region "$REGION" \
  --member="serviceAccount:$SCHEDULER_SERVICE_ACCOUNT" --role=roles/run.invoker --quiet
if gcloud_as_deploy scheduler jobs describe wrkflw-cleanup --location "$REGION" >/dev/null 2>&1; then
  scheduler_action=update
else
  scheduler_action=create
fi
gcloud_as_deploy scheduler jobs "$scheduler_action" http wrkflw-cleanup \
...
effective_scheduler_identity="$(gcloud_as_deploy scheduler jobs describe wrkflw-cleanup --location "$REGION" --format='value(httpTarget.oauthToken.serviceAccountEmail)')"
