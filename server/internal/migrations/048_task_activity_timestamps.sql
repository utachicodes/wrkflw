...
CREATE OR REPLACE FUNCTION track_task_activity_timestamps()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
  record_run boolean := false;
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.status = 'done' AND NEW.completed_at IS NULL THEN
      NEW.completed_at = now();
    END IF;
    record_run := NEW.execution_run_id IS NOT NULL;
  ELSE
    ...
    END IF;
    record_run := NEW.execution_run_id IS NOT NULL
      AND NEW.execution_run_id IS DISTINCT FROM OLD.execution_run_id;
  END IF;
...
CREATE TRIGGER tasks_track_activity_timestamps
BEFORE INSERT OR UPDATE OF done, status, execution_run_id ON tasks
FOR EACH ROW EXECUTE FUNCTION track_task_activity_timestamps();
